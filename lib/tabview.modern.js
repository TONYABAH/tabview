...
const TabView = function () {
  ...
  function createLinkButton(linkId, tabId, {
    target,
    style,
    title,
    bg,
  ...
  const createTabs = function createTabs(el, options) {
    var selectedTab, tabBar, rightBtn, leftBtn, addBtn, pagePanelId, tabBarId, themeOptions, tabSettings, containerId, dragStartPoint, dragStopPoint;
    const tabs = new Map();
    const buttons = {};
    init(el, options);
    /**
    ...
    function init(el, options) {
      ...
      const doc = createTable(tabBar.parentElement, pagePanel, themeOptions);
    ...
    function unhide() {
      ...
      const gap = tabBar.getBoundingClientRect().right - last.getBoundingClientRect().right;
    ...
    function add(options = {}) {
      ...
      var linkButton = createLinkButton(buttonId, tabId, tabSettings); // buttons[linkButton.id] = linkButton;
      ...
      buttons[linkButton.id] = linkButton; // If there was a selected button already, insert after it
      // otherwise append at the end of tab list

      if (selectedTab = getSelectedTab()) {
        // tab.selected=false;
        selectedTab.linkButton.after(linkButton); // getLinkButton(tab.linkId).after(linkButton);
      } else {
