...
#${containerId} .tab-header {
  ...
  function createIcon(icon, iconColor) {
    ...
    btn.style.left = '4px'; // btn.style.top = '50%';
  ...
  function createTabs(containerId, options = {
    ...
    let tabAlignSide = options.align === 'left' || options.align === 'right' ? true : false;
    let infiniteScroll = options.infiniteScroll || false;
    let activeTab = null;
    let tabBar = null; // container.querySelector('.tabs');

    let tabPanel = null; // let tabs = tabBar.querySelector('.tabs');
    ...
    if (container.classList.contains('tab-align-right') || container.classList.contains('tab-align-left')) {
      tabAlignSide = true;
    }
    ...
    function createButton(tabId, options) {
      ...
      btn.addEventListener('click', e => selectTab(btn.getAttribute('data-tab')), true);
    ...
    function addTab(tabId, options = {
      ...
      tab.options = options;
      tabs.push(tab);
      tabPanel.appendChild(vpage); // let selectedTab = tabs.find((t) => t.active === true);
    ...
    function createTab(tabId, options = {
      ...
      delete options.attributes;
      tab.options = options;
      tabs.push(tab); // tab.onClick = (fn) => btn.addEventListener('click', fn);
    ...
    const closeTab = debounce(tabId => {
      ...
      if (tabButton.nextElementSibling) {
        nextButtonId = tabButton.nextElementSibling.getAttribute('data-tab');
      } else if (tabButton.previousElementSibling) {
        nextButtonId = tabButton.previousElementSibling.getAttribute('data-tab');
      }
    ...
    const selectTab = debounce(tabId => {
      let tab = tabs.find(t => t.id === tabId);
      if (tab) return; // let currentTab = tabs.find((t) => t.active === true);
      ...
      events.fire('select', tab); // tab.select();
    }, 250, true);
