const TabView = function () {
  ...
  const addCSS = (containerId, stylesId, options = {}) => {
    ...
    const s = Object.assign(DEFAULT_THEME_SETTINGS, DEFAULT_LAYOUT_SETTINGS, options); // const s = Object.assign(THEME_SETTINGS, LAYOUT_SETTINGS);
    ...
    const panelTop = options.tabPosition !== 'left' && options.tabPosition !== 'right' && options.tabPosition !== 'bottom' ? headerHeight + s.tabHeight : headerHeight;
    const panelBottom = options.tabPosition === 'bottom' ? footerHeight + s.tabHeight : footerHeight;
    const css = `
    #${containerId} {
...
#${containerId} .tab-header {
  ...
  const dnd = {
    ...
    drop: ev => {
      ...
      var target = role === 'tab-button' ? ev.target : ev.target.parentNode; // getLinkTarget(ev.target);

      const dataEl = document.getElementById(data);
      const dragStartPoint = dataEl.getBoundingClientRect();
      const dragStopPoint = target.getBoundingClientRect();

      if (dragStartPoint.x > dragStopPoint.x || dragStartPoint.y > dragStopPoint.y) {
  ...
  function getVisibleTabs(tabBar) {
    ...
    const visibleTabs = []; // const invisibleTabs = [];
  ...
  function createCloseButton() {
    ...
    btn.classList.add('close-button'); // btn.style.position = 'absolute';
    // btn.style.right = '4px';
    // btn.style.top = '50%';
  ...
  function createIcon(icon, iconColor) {
    ...
    btn.style.position = 'absolute'; // btn.style.left = '4px';
    // btn.style.top = '50%';

    btn.style.color = iconColor || ''; // btn.style.transform = 'translateY(-50%)';
  ...
  function createPage(pageId, content) {
    ...
    page.setAttribute('id', pageId); // page.textContent = text;
  ...
  function createTabs(containerId, options) {
    ...
    const tabs = [];
    const draggable = options.sortable || container.getAttribute('sortableTabs');
    const align = options.tabPosition || container.getAttribute('tabPosition');
    const tabAlignSide = align === 'left' || align === 'right' ? true : false;
    const infiniteScroll = options.infiniteScroll || container.getAttribute('infiniteScroll');
    ...
    function handleKeypress(e) {
      ...
      const link = activeTab.linkButton; // eslint-disable-next-line default-case
    ...
    function createButton(tabId, options) {
      ...
      title = title || text; // let closable = tab.closable;
      ...
      btn.setAttribute('role', 'tab-button'); // const label = createLabel(text);
    ...
    function openTab(tabId, options = {
      ...
      const vpage = createPage(pageId, options.content); // tab.attributes.forEach((a) => btn.setAttribute(a[0], a[1]));
    ...
    function createTab(tabId, options = {
      attributes: [],
      iconColor: 'red'
    }) {
      const btn = createButton(tabId, options);
    ...
    const closeTab = debounce(tabId => {
      ...
      if (tab.linkButton.nextElementSibling) {
        nextButtonId = tab.linkButton.nextElementSibling.getAttribute('data-tab');
      } else if (tab.linkButton.previousElementSibling) {
        nextButtonId = tab.linkButton.previousElementSibling.getAttribute('data-tab');
      }
    ...
    const selectTab = tabId => {
      let tab = tabs.find(t => t.id === tabId);
      if (!tab) return;
      let ce = events.fire('before-tab-select', {
        oldTab: activeTab,
        newTab: tab
      }, tabBar);
      ...
      events.fire('tab-select', tab, tabBar); // tab.select();
    };
    ...
    function selectPage(pageId) {
      ...
      let page = document.getElementById(pageId); // if (activeTab) activeTab.tabPage.style.display = 'none';
    ...
    const scroButtonToView = () => {
      ...
      if (visibleTabs.length > 1) {
        ...
        if (last.nextElementSibling) {
          if (tabAlignSide) {
            visibleTabs[0].style.marginTop = '-' + visibleTabs[0].clientHeight + 'px';
          } else {
            visibleTabs[0].style.marginLeft = '-' + visibleTabs[0].clientWidth + 'px';
          }
        }
      }
    };
    ...
    function selectButton(id) {
      ...
      if (!el) return;
      let target = el.getAttribute('role') === 'tab-button' ? el : el.parentNode;
    ...
    const scrollLeft = debounce(() => {
      ...
      if (visibleTabs.length >= 1) {
        let last = visibleTabs[visibleTabs.length - 1]; // if (last.nextElementSibling) {
    ...
    const scrollRight = debounce(e => {
      ...
      if (visibleTabs.length >= 1) {
        let first = visibleTabs[0]; // if (first.previousElementSibling) {
        ...
        if (infiniteScroll) {
          ...
          if (tabAlignSide) {
            if (first.previousElementSibling) first.previousElementSibling.style.marginTop = 0;
          } else {
            if (first.previousElementSibling) first.previousElementSibling.style.marginLeft = 0;
          }
        }
      }
    }, 50);
    ...
    function init() {
      ensureMarkup(); // let tabBarWrapper = container.querySelector('.tab-bar');
    ...
    const onBeforeSelect = fn => events.on('before-tab-select', fn, tabBar);
